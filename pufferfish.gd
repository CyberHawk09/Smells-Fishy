...
func _on_area_entere2d(area):
	print("Detected Node: ", area.name)
	print("Node Groups: ", area.get_groups())
	print("Parent Groups: ", area.get_parent().get_groups())
	
	
	if area.name == "Hook":
		caught = true
		hook = area
	# If the group is on the parent, this will work:
	if area.get_parent().is_in_group("your_group"):
		print("Found group on parent!")
